...
import { CartDelete, DecriseQuantity, IncriseQuantity } from "@/redux/cardSlice";
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-12">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <Link href={'/'} className="flex items-center text-gray-600 hover:text-green-600 mb-6 transition-colors font-medium">
          <ArrowLeft className="w-5 h-5 mr-2" /> Back to Home
        </Link>

        <div className="flex items-center gap-3 mb-8">
          <ShoppingCart className="w-8 h-8 text-green-600" />
          <h1 className="text-3xl font-bold text-gray-800">Your Shopping Cart  {cardData.length}</h1>
        </div>
              ...
              {cardData.map((item:IGrocery,index) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <Image src={item.image} alt={item.name} height={160} width={160}/>
                    </div>
                    <div>
                      <h3 className="font-semibold text-lg">{item.name}</h3>
                      <p className="text-sm text-gray-500">{item.unit}</p>
                      <p className="font-bold text-green-600 mt-1 text-lg"><span className="text-lg mr-1 font-black">৳</span>{item.price}</p>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between text-gray-600">
                  <span>Subtotal</span>
                  <span className="font-semibold"><span className="text-md mr-1 font-black">৳</span>20</span>
                </div>
                <div className="flex justify-between text-gray-600">
                ...
                <div className="border-t pt-4 flex justify-between items-center">
                  ...
                  <span className="text-2xl font-bold text-green-600"><span className="text-2xl mr-1 font-black">৳</span>30</span>
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full mt-4 text-red-500 font-medium hover:underline"
              >
                Clear Cart
              </button>
            </motion.div>
